...
formulario.addEventListener('submit', async e => {
    ...
    if (resultado.codigo === 201) {
        const URLactual = window.location.host;
        // console.log(os.hostname());
        // construir un mensaje de que todo se creo bien
        const mensaje = document.createElement('div');
        mensaje.classList.add('mensaje-url');
        mensaje.innerHTML = `<p>Se ha acortado correctamente la URL, vísita <a target="_blank" rel="" href="/${resultado.url}"> ${URLactual.toLowerCase() + '/' + resultado.url
            }</a > </p > `;
